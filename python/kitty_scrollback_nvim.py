...
def handle_result(args: List[str],
    ...
    if w is not None:
        ...
        nvim_args = parse_nvim_args(args) + (
            ...
            '    local ok, ksb_launch = pcall(require, [[kitty-scrollback.launch]])'
            '    if ok then'
            f'    ksb_launch.setup_and_launch([[{kitty_data}]])'
            '    else'
            '     local err_msg = {'
            '      [[]],'
            '      [[==============================================================================]],'
            '      [[kitty-scrollback.nvim]],'
            '      [[]],'
            '      [[ERROR: failed to load kitty-scrollback.nvim]],'
            '      [[]],'
            '      [[>]],'
            '     }'
            '     for line in ksb_launch:gmatch([[[^\r\n]+]]) do'
            '      table.insert(err_msg, [[  ]] .. line)'
            '     end'
            '     vim.list_extend(err_msg, {'
            '      [[<]],'
            '      [[INFO]],'
            '      [[- vim.fn.getcwd(): ]] .. vim.fn.getcwd(),'
            f'     [[- os.getcwd(): {os.getcwd()} ]],'
            f'     [[- inspect.getfile(lambda: None): {inspect.getfile(lambda: None)} ]],'
            f'     [[- ksb_dir: {ksb_dir}]],'
            '      [[- kitty_data:]],'
            '      [[>]],'
            f'     [[  {kitty_data}]],'
            ...
            '     vim.list_extend(err_msg, {'
            '      [[<]],'
            '      [[- vim.api.nvim_get_runtime_file(lua/kitty-scrollback/*.lua):]],'
            '      [[>]],'
            '     })'
            '     vim.list_extend('
            '      err_msg,'
            '      vim.tbl_map(function(v)'
            '       return [[  ]] .. v'
            '      end, vim.api.nvim_get_runtime_file([[lua/kitty-scrollback/*.lua]], true))'
            '     )'
            '    '
            ...
            '      buf = error_bufid,'
            '     })'
            '     vim.api.nvim_buf_set_lines(error_bufid, 0, -1, false, err_msg)'
            '    end'
            '  end, '
            ' })')
