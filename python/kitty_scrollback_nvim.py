...
def start_nvim_server(socket_path):
    debug(f'starting nvim on {socket_path} ...')
    try:
    ...
    return int(
        subprocess.check_output([
            ...
            '-n',
            '-S',
            f'{ksb_dir}/lua/scrollback/launch.lua',
        ]))
...
def pipe_data(w, lines: int):
    input_line_number = (lines - (w.screen.lines - 1) - w.screen.scrolled_by)
    return {
...
def handle_result(args: List[str], result: Dict[str, Any],
    ...
    if w is not None:
        pd = json.dumps(pipe_data(w, request_id))
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
            if not connect_to_nvim_server(client, socket_path):
                debug('could not connect to nvim')
                return
