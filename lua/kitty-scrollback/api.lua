...
M.generate_kittens = function(all)
  ...
  local kitty_scrollback_configs = vim.api.nvim_get_runtime_file('lua/kitty-scrollback/configs', false)[1]

  local ksb_configs = { '' }
  local action_alias = 'kitty_scrollback_nvim'
  local top_ksb_configs = {
    '# kitty-scrollback.nvim Kitten alias',
    'action_alias ' .. action_alias .. ' kitten ' .. kitty_scrollback_kitten .. ' --cwd ' .. kitty_scrollback_configs,
    '',
  ...
  for _, c in pairs(vim.api.nvim_get_runtime_file('lua/kitty-scrollback/configs/*.lua', true)) do
    ...
    local config = (keymap or 'map f1') .. ' ' .. action_alias .. ' --config-file ' .. name .. '.lua'
  ...
  local kitten_configs = vim.list_extend(
    vim.list_extend(
      vim.tbl_extend('force', top_ksb_configs, {}),
      ksb_configs
    ),
    nvim_args
  )
